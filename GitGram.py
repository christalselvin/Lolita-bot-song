...
def support(update, context):
    """Support command"""
    update.message.reply_text("For support, visit our support group.")

def source(update, context):
    """Source command"""
    update.message.reply_text(f"Source code available at {GIT_REPO_URL}")
...
# Function to post messages to Telegram groups
def post_tg(chat, message, parse_mode):
    """Send message to the desired group"""
    TG_BOT_API = f'https://api.telegram.org/bot{BOT_TOKEN}/sendMessage'
    response = post(
        TG_BOT_API,
        params={
            "chat_id": chat,
    ...
    log.info(f"Attempting to send message to chat_id: {chat}")
    log.info(f"Message content: {message}")
    log.info(f"Response from Telegram API: {response}")
    return response


# Flask routes for handling webhooks
